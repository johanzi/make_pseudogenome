...
# Pipeline
Two methods are suggested here to generate a pseudogenome. The first method is quicker and replaces all positions of low quality, missing data, or deletions in the VCF file by the reference allele while the second method replaces these positions by Ns by using a mask. The latter method yields a more correct pseudogenome *sensu stricto* but generates pseudogenomes with often lower mappability as the assumption of the reference allele is often correct when data is missing or of low quality. For applications related to mapping short reads (BS-seq, DNase-seq, ChIP-seq, ...), the first method is advised.

## Behaviour regarding indels
For instance, in the case of a 4-bp deletion with a good quality call:

Reference allele: AGCCC

Alternative allele: A

**Pseudogenome generated**

First method (without mask): 
* Deletion is ignored (*--remove-indels*) => AGCCC (chromosome size unchanged)
* Deletion is integrated in the pseudogenome (no *--remove-indels*)  => A (chromosome size changed)

Second method (with mask):
* If deletion is not excluded to the mask (no *--exclude-deletion* option)  => ANNNN  (chromosome size unchanged)
* If deletion is excluded from the mask and therefore integrated in the pseudogenome (*--exclude-deletion* option) => A (chromosome size changed)

In conclusion, when the chromosome size should stay unchanged, the second method allows to visualize deletion as Ns while the first method does not.

## Method 1 - without mask
### Step 1: Subset VCF file with indels
This step allows to process faster the downstream steps as computational time increases with the size of the VCF file used. Only positions containing at least 1 alternative allele is kept (--min-alleles 2) allowing to generate a much smaller VCF file. The tool *bcftools consensus* assumes reference allele when no data are available so that positions removed will be *de facto* considered as reference. The vcf file should be compressed with bgzip and indexed with tabix. The samples_id.txt should contain the accessions ID (matching in VCF), one line per accession. If needed to check samples ID, type "bcftools query -l <file.vcf.gz>". One can directly  filter the VCF file to keep only positions with a certain quality threshold. If the chromosome lengths should stay identical to the reference in the pseudogenome, indels should be removed from the VCF file. If indels should be integrated, changing chromosome sizes, perform the following command without the option *--remove-indels*. 
...
### Step 2: Generate VCF file without indels
...
```
python filter_vcf2bed.py -s sample_name -i <subset_vcf_file.vcf.gz> > mask_sample_with_deletions.bed
```
...
```
python filter_vcf2bed.py -s sample_name -i <subset_vcf_file.vcf.gz> -q 30 -c 5 > mask_sample_with_deletions.bed
```
...
```
python filter_vcf2bed.py --exclude-deletions -s sample_name -i <subset_vcf_file.vcf.gz> > mask_sample_wo_deletions.bed
```
...
Merge mask_sample_with_deletions.bed (or mask_sample_wo_deletions.bed) file:
```
bedtools merge -i mask_sample_with_deletions.bed > mask_sample_with_deletions.merged.bed
```

Rename and overwrite initial unmerged bed file:
```
mv mask_sample_with_deletions.merged.bed mask_sample_with_deletions.bed
```
...
The mask will enable to replace by Ns all bases with a bad quality call and the deletions so that chromosome sizes stay equal to reference.
```
bcftools consensus <subset_vcf_file_wo_indels.vcf.gz> --sample sample_name --fasta-ref <reference_fasta.fa> --mask <mask_sample_with_deletions.bed> > pseudogenome_sample_name.fa
```

### 2. Generate pseudogenome with deletions
In this case, the mask bed file was generated using the option *--exclude-deletions* to allow deletions passing quality check to be included in the pseudogenome (changing chromosome size):
```
bcftools consensus <subset_vcf_file.vcf.gz> --sample sample_name --fasta-ref <reference_fasta.fa> --mask <mask_sample_wo_deletions.bed> > pseudogenome_with_deletions_sample_name.fa
```
